...
router.post("/signin", async (req, res) => {
  ...
  if (!email || !password)
    return res.status(400).send("Invalid email or password."); //custom validate
  ...
  User.findOne({ email: email }).then((user) => {
    ...
    bcrypt
      ...
      .then((isMatch) => {
        if (isMatch) {
          const newtoken = user.generateAuthToken();
          const { _id, name, email, followers, following, pic, role } = user;
          console.log(newtoken);
          res.json({
            newtoken,
            user: { _id, name, email, followers, following, pic, role },
          });
        } else {
